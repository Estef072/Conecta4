...
function evaluate(board) {
    ...
    for (let row of board) {
      for (let cell of row) {
        if (cell === 1) {
          score += 10; // Aumentar el valor de las fichas del jugador 1
        } else if (cell === 2) {
          score -= 10; // Disminuir el valor de las fichas del jugador 2
        }
      }
    }
...
function getBestMove(board, maxEval, maximizingPlayer) {
    ...
    for (let column = 0; column < 7; column++) {
      if (isValidMove(board, column)) {
        ...
        const opponentMove = getBestMove(newBoard, maxEval, !maximizingPlayer);
        if (opponentMove === -maxEval) {
          // Bloquear movimientos del oponente que lleven a la victoria inminente
          continue;
        }
