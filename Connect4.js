...
function Alphis_beta(board, depth, alpha, beta, maximizingPlayer) {
  ...
  if (maximizingPlayer) {
    ...
    for (let column = 0; column < 7; column++) {
      if (isValidMove(board, column)) {
        ...
        const eval = Alphis_beta(newBoard, depth - 1, alpha, beta, false);
    ...
    for (let column = 0; column < 7; column++) {
      if (isValidMove(board, column)) {
        ...
        const eval = Alphis_beta(newBoard, depth - 1, alpha, beta, true);
...
socket.on('ready', function (data) {
  ...
  const result = Alphis_beta(clonedBoard, depth, alpha, beta, maximizingPlayer);
...
function getBestMove(board, maxEval, maximizingPlayer) {
  ...
  for (let column = 0; column < 7; column++) {
    if (isValidMove(board, column)) {
      ...
      const eval = Alphis_beta(newBoard, 0, -INF, INF, !maximizingPlayer);
      if (eval === maxEval) {
